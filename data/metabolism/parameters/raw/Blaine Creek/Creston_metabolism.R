...
blaine_65<-read.csv("./blaine_65_oxy.csv") #this is a chopped down PME file.
...
plot(blaine_params$K600.daily, blaine_params$ER.daily)

plot(blaine_params$GPP.daily, blaine_params$ER.daily)
...
lightinaday<-blaine_65$ligh[100:244]/sum(blaine_65$ligh[100:244])

NEP<- mean(blaine_params$GPP.daily, na.rm=T)*lightinaday +  mean(blaine_params$ER.daily, na.rm=T)*(15/1440)

NEP_mmol<- (1440/15)*NEP*1000/32
plot(blaine_65$solar.time[100:244], -NEP_mmol, type="l", ylim=c(-300,600), xlab="Solar time", ylab= "-NEP mmol m-2 d-1" )
